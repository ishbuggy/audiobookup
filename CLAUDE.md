...
### Task tracker (Forgejo issues)
- Forgejo issues in `homelab/audiobookup` (https://git.torontontero.com/homelab/audiobookup/issues) are the canonical task list and the capture inbox for user feedback, observations, and improvement ideas not yet scheduled into a release. When feedback arrives mid-session, or a `ref-docs/user-reports/` triage surfaces actionable items, file an issue rather than losing it in conversation or scattering TODOs. Cross-cutting infra work that is not about this app goes to `homelab/tasks`.
- Filing, closing, and the release sweep are owned by the `issues` skill; invoke it rather than calling the API by hand. It owns the label rules and the milestone semantics. Mechanics for reference: API base `https://git.torontontero.com/api/v1`, token at `~/.config/forgejo/token`, `gh` is not installed, use `curl`.
...
- **Executing one phase of `PLAN.md` is owned by the `implement-phase` skill** — invoke it in a fresh session rather than driving the phase by hand. It reads the phase *and the numbered decisions it cites*, front-loads the phase's tail-end obligations (review tier, issue closes, report path, docs touched) into a checklist **before any code is written**, then delegates the build, runs `review-pass`, commits, and calls `phase-report`.
- The front-loading is the whole point: the review, the issue close and the report all fall due at the *end* of a phase, by which time the plan text carrying them is hundreds of lines back in a context full of implementation work. That is the failure mode, not forgetting the rules.
...
- **Pick the subagent's model by task type** rather than inheriting the default:
  ...
  - **Sonnet** — document writing and updating (readme, CHANGELOG, ROADMAP, plan and report write-ups).
  ...
  - **Prompting Fable 5.1** (planning sessions and major-checkpoint reviews; distilled from Anthropic's Fable 5.1 prompting guide — https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5-1). Fable 5.1 is a drop-in replacement for Fable 5 — Fable 5 is never used any more:
    - **Give the reason, not only the request** — Fable performs better when it knows the intent behind the task (what the release is for, what the phase set out to prove), so a kickoff or review prompt carries the why alongside the range.
    - **State the boundaries explicitly** — Fable occasionally takes unrequested actions, so a findings-only or plan-only mandate must be written into the prompt, never assumed. The mirror image also has to be stated: a dispatch for autonomous work says the operator is not watching, work already requested needs no permission, and the turn ends only when the work is done.
    - Brief instructions steer it — don't enumerate behaviors or port Opus-era hand-holding into Fable prompts; project facts and orderings (archive-first, path resolution) stay, micro-instructions about how to think go. Never instruct it to echo or transcribe its reasoning — that can trigger a reasoning-extraction refusal. Adversarial framing runs on Opus, never Fable — unchanged for 5.1.
    - **Effort:** high is the starting point; reserve xhigh/max for a measured gain — at those levels a long deliverable (`PLAN.md`, `REVIEW.md`) can be drafted in thinking and then again in the reply.
    - **No extras, surgical edits:** 5.1 tends to fix nearby code, extend behaviour the task didn't name, and add test files beyond what the change warrants, and it rewrites whole files more readily than Fable 5. A fix dispatch says: fix only what the task names, report the rest as follow-ups, commit tests only where the task asks or the repo already keeps them, and edit surgically where the end result is the same.
    - **Narration and prose:** it says less between tool calls than Fable 5 — when visibility matters, ask for a line up front and a standalone recap at the end. It under-formats rather than over-formats, so drop anti-bullet rules; if a plan or review reads dense, "remove all mannered prose" fixes it.
