...
- **Long MP3 conversions no longer time out on slow machines:** The time limit for an MP3 conversion was based on a formula meant for the faster default format, so a very long book converting to MP3 on modest hardware could be declared "stuck" and cancelled while it was actually working fine. The MP3 limit is now based on the book's own length. And when a conversion genuinely does run out of time, the encoder is now actually stopped instead of being left running in the background — and a conversion that fails partway no longer leaves a stray leftover file in your library folder.
- **A hand-edited sample-rate limit can't silently break MP3 encoding:** If the MP3 sample-rate cap in the settings file had been hand-edited to something other than a whole number, every MP3 conversion would fail with a confusing encoder error. Such values are now handled sensibly instead.
- **Assorted small fixes:** A clearer error message when the "drop zero-length chapters" cleanup removes every chapter a book has; companion files with uppercase extensions (like `.JPG`) are now recognized when a book is renamed, re-dated, or cleaned up; and a couple of hard-to-reach edge cases in file naming and duplicate marking are handled properly.
