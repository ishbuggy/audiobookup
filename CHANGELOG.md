...
### Added
- **Split books into one file per chapter:** A new "Split by Chapter" option (off by default) saves each chapter of a book as its own file inside a folder for that book, instead of one big audiobook file — handy for players that handle multi-file books better than embedded chapters. Each file is fully tagged (book details, its chapter's title, track numbers, and the cover), and with the default naming template the files are numbered so they sort in playback order — a customized template can change that. A companion setting merges very short chapters (like Audible's "Chapter 1" announcement stubs) into the chapter that follows so you don't end up with three-second files. A customizable naming template controls how the chapter files are named. It works with all three output formats: M4B, MP3, and Original quality. With Original quality the chapters are usually cut straight out of the downloaded audio without re-encoding it, so splitting costs almost nothing, loses no quality, and lands within a few milliseconds of each chapter mark; a small number of titles need a compatibility decoding step during conversion, so those chapters get re-encoded instead of cut untouched — they still come out as M4B files either way. Two format notes: split books don't get a `.cue` sheet (with one file per chapter, there's nothing for it to add), and splitting to MP3 adds a few milliseconds of silence at each chapter boundary, which is simply how MP3 works. This applies to books you convert from now on — a book you already have keeps its current single-file shape until you Force Re-download it.
- **See a split book's files at a glance:** Books that were split by chapter show an "N files" badge in every library view, and their detail view lists the book's folder, how many chapter files it contains and their total size, with an expandable list of the individual files. The Force Re-download confirmation also spells out that a split book's chapter files are replaced as a set. The new options live in Settings under Chapters & Metadata and File & Folder Naming — "Split by Chapter" itself is a standard option, while its merge threshold and the chapter file naming template are advanced ones.
- **Split books are managed like any other book:** Renaming one moves its whole folder, and a Force Re-download cleans up the previous files even when the book switches between split and single-file or between formats. The library scan and the integrity check look at every chapter file, so a split book counts as complete only while all of its files are there — the integrity check's error spells out exactly how many files are missing, while a routine library sync just flags the book as missing without that detail — and if you move a split book's folder somewhere else in your library, keeping all of its chapter files together, the next full library scan can still find it and re-link it, though it takes a cleaner move to recognize than a single-file book does. Your own chapter files are never mistaken for stray files to import, the Download Annotations button keeps working, and two books that would land on the same folder name get separate folders automatically. One note: renaming a split book updates its folder but keeps the individual chapter files' names until the next re-download.

### Fixed
- **Renaming a book can no longer take another book's companion files with it:** In the rare case where two books share the same name apart from their file extension (possible in libraries from before the same-name protections, or via manual upload), renaming one of them also moved the other's cover, PDF, and chapter files away with it. The rename now leaves shared companion files alone, the same way the post-download cleanup already does.
- **Manual uploads can no longer quietly share files with an existing book:** Uploading a book whose name matched an existing book's except for the file extension (say, an `.m4a` next to an existing `.m4b`) placed the two side by side, where they would share cover art and other companion files — a mix-up the previous release fixed for downloads and renames but noted was still possible through upload. Uploads now get their own name in that situation, closing the gap.
- **A finished conversion can no longer be marked as failed by after-the-fact housekeeping:** Once a book has converted successfully and been recorded in the library, a hiccup in the follow-up chores (saving companion files, setting file dates, cleaning up files from a previous download) is now just noted in the log instead of marking the whole book as an error — previously a rare glitch there could flag a perfectly good download as failed.
...
- **Importing an old settings backup can no longer change your login password:** Settings files exported before v0.23.0 carried the (hashed) web login password, and importing one silently switched your password back to whatever it was when the export was made — with no warning until your next login. Imports now ignore that credential (and the internal setup marker), matching what exports have left out since v0.23.0; changing your password through the settings page works exactly as before.
- **On-demand annotations files get the same file date as the rest of the book:** With the "File Timestamp Source" option set to the book's release or purchase date, an annotations file fetched later with the "Download Annotations" button kept the moment of the download as its file date, standing out from the book's other files. It's now stamped to match its siblings.
- **Chapter-listing (`.cue`) files survive unusual titles:** A book or chapter title containing a quotation mark or a line break could corrupt the optional `.cue` companion file (the previous safeguard used an escape the CUE format doesn't actually define). Quotes are now replaced with two apostrophes and line breaks with a space, keeping the file valid.
- **Buttons that could only ever fail are hidden for imported books:** Books added by manual import or found by the library scan have no Audible identity, so the "Get Full Summary" and "Download Annotations" buttons — which ask Audible for the book's data — could never succeed for them and always ended in an error. Both buttons are now shown only for books that came from your Audible library.
- **Two detail-view buttons now match the app's look:** The "Get Full Summary" and "Download Annotations" buttons in a book's detail view previously rendered as plain, unstyled browser buttons; they now share the styling of the app's other secondary buttons in both light and dark mode.
- **A too-short password is caught before saving, and settings errors explain themselves:** Choosing a new web login password shorter than 8 characters is now stopped right in the browser with a clear message, instead of failing with a generic error after the save was attempted. And when saving settings fails for any other reason, the server's actual explanation is now shown in place of the one-size-fits-all "Server returned an error."
- **Converting a book to MP3 works again:** Since v0.22.0, choosing MP3 as the output format failed every conversion outright — the encoder was being handed a file it didn't know how to write and stopped before producing any audio. MP3 conversions now complete normally again.
...
### Note
- **Chapter boundaries are only as accurate as Audible's own chapter data:** on some titles, the chapter markers Audible provides are off by a few seconds. That's not new, but it's more noticeable with Split by Chapter on, since a small misalignment now falls right at the start or end of a file instead of being buried inside one long track — you might occasionally hear a word clipped at the very start or end of a chapter file. This is a quirk in Audible's own chapter data, not something splitting introduces, and it isn't fixed in this release.
...
- **Chapter marks now stay within the book's audio:** With the branding-trim option enabled, a book whose chapter data places a marker inside Audible's closing branding could end up with a bit of that outro audible at the end of the last chapter. Those markers are now removed, and — with or without the branding trim — chapter timings that overrun the end of the audio are now trimmed back so no chapter mark points past the end of the book.
- **Cancelling a conversion now reliably stops MP3 encoding:** If a conversion to MP3 was cancelled at exactly the wrong moment — just as the book's audio properties were being read — the long MP3 encoding step could still start and run all the way to the end, tying up the machine for work that was then thrown away. Cancellation is now caught before the encoder starts.
- **Failed downloads no longer retry forever:** Scheduled auto-processing promised to retry a failed book only once, but actually kept retrying it on every run, indefinitely — repeatedly asking Audible for a book that could never succeed. A failed book is now retried automatically exactly once, as the setting always promised, and retrying a book by hand still works anytime (and re-arms one more automatic attempt). One transition note: a book that had already failed before this release may receive up to two final automatic attempts after updating; after that, the once-only rule holds.
- **A mistyped new password can no longer lock you out:** Changing the web login password now requires the New Password and Confirm New Password fields to match — if they don't, nothing is saved and an error explains why. Previously the New Password field was saved as typed even when the confirmation didn't match, quietly setting a password you didn't intend and locking you out at your next login.
- **Settings exports no longer include your login credential:** The settings "Export as JSON" backup no longer contains the (hashed) web login password, so an export can be shared — for example when asking for help — without handing out a credential someone could try to crack offline. Exports import exactly as before. Files exported by older versions still import fine too — but they carried the credential, so treat those older files as private, and be aware that importing one also switches your login password back to whatever it was when that export was made.
- **Long MP3 conversions no longer time out on slow machines:** The time limit for an MP3 conversion was based on a formula meant for the faster default format, so a very long book converting to MP3 on modest hardware could be declared "stuck" and marked as failed while it was actually working fine. The MP3 limit is now based on the book's own length. And a few unusual failure cases that could previously leave a stray partial file behind in your library folder now clean up after themselves too.
- **A hand-edited sample-rate limit can't silently break MP3 encoding:** If the MP3 sample-rate cap in the settings file had been hand-edited to a decimal or true/false value, MP3 conversions would fail with a confusing encoder error. Such values are now handled sensibly instead.
- **Assorted small fixes:** A clearer error message when a book ends up with no usable chapters after its chapter data is processed; companion files with uppercase extensions (like `.JPG`) are now recognized when a book is renamed, re-dated, or cleaned up; a settings toggle with a long description no longer squishes and clips its on/off switch; and a couple of hard-to-reach edge cases in file naming and duplicate marking are handled properly.
