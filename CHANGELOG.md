...
- **Library UI Polish:** A handful of small front-end rough edges were fixed. Replacing a book's cover now shows the new art even after closing and reopening the detail modal (the cached original was previously served until a hard refresh). A partial bulk rename now keeps the books that failed selected — with the toast noting they're still selected — so you can retry them without re-picking, instead of clearing the whole selection. The large-download confirmation falls back to a plain browser prompt if the styled dialog is unavailable, and toast notifications now render their message as text, so an error string can never inject markup.
