...
### Added
- **Split books into one file per chapter:** A new "Split by Chapter" option (off by default) saves each chapter of a book as its own file inside a folder for that book, instead of one big audiobook file — handy for players that handle multi-file books better than embedded chapters. Each file is fully tagged (book details, its chapter's title, track numbers, and the cover), files are numbered so they always sort in playback order, and a companion setting merges very short chapters (like Audible's "Chapter 1" announcement stubs) into the chapter that follows so you don't end up with three-second files. A customizable naming template controls how the chapter files are named. It works with all three output formats: M4B, MP3, and Original quality. With Original quality the chapters are usually cut straight out of the downloaded audio without re-encoding it, so splitting costs almost nothing, loses no quality, and lands within a few milliseconds of each chapter mark; a small number of titles need the compatibility download path, and those are split into M4B files instead. Two format notes: split books don't get a `.cue` sheet (with one file per chapter, there's nothing for it to add), and splitting to MP3 adds a few milliseconds of silence at each chapter boundary, which is simply how MP3 works.
- **See a split book's files at a glance:** Books that were split by chapter show an "N files" badge in every library view, and their detail view lists the book's folder, how many chapter files it contains and their total size, with an expandable list of the individual files. The Force Re-download confirmation also spells out that a split book's chapter files are replaced as a set. The new options live in Settings — "Split by Chapter" under Chapters & Metadata, with the merge threshold and the chapter file naming template alongside the other advanced options.
- **Split books are managed like any other book:** Renaming one moves its whole folder, and a Force Re-download cleans up the previous files even when the book switches between split and single-file or between formats. The library scan and the integrity check look at every chapter file, so a split book counts as complete only while all of its files are there and is flagged as missing or damaged with a count of how many turned up — and if you move a split book's folder somewhere else in your library, the next full library scan finds it again and follows it there, just as it always has for single-file books. Your own chapter files are never mistaken for stray files to import, the Download Annotations button keeps working, and two books that would land on the same folder name get separate folders automatically. One note: renaming a split book updates its folder but keeps the individual chapter files' names until the next re-download.
...
- **Converting a book to MP3 works again:** Since v0.22.0, choosing MP3 as the output format failed every conversion outright (unless the new per-chapter splitting option was on) — the encoder was being handed a file it didn't know how to write and stopped before producing any audio. MP3 conversions now complete normally again.
