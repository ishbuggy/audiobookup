...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if "username" not in session:
            # --- FIX: Manually build a relative path for the 'next' parameter ---
            # This avoids using request.url (which causes WAF errors) and potential
            # issues with request.full_path, by constructing the path explicitly.
            next_path = request.path
            if request.query_string:
                # Safely append the query string if it exists.
        ...
        if not os.path.exists(SETUP_FLAG_FILE):
            ...
            if request.endpoint not in ["setup", "logout", "static"] and not request.path.startswith(
                "/setup/socket.io"
            ):
