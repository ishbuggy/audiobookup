...
- **Smart File Management:**
    ...
    - **File Timestamp Source:** Optionally stamp each finished audiobook (and its companion files) with the book's release date or the date you purchased it, instead of the time it was downloaded, so file browsers and media servers can sort by it. Applies to books you download or convert from now on — books already on disk keep their existing file dates.
