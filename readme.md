...
- **Settings Configuration:** Configure features from a dedicated `/settings` page, split into a **Standard** view for everyday options (output format, quality, naming, cover art, and more) and a single **Advanced Mode** toggle that reveals the full power-user surface — download-quality requests, MP3/LAME tuning, chapter and metadata cleanups, sidecar files, and manual control over `Total Processing Cores` / `Max Parallel Downloads` (vs. Standard mode's CPU auto-detection) — without cluttering the page for everyday use.
