...
Treat this as a read-only inspection tool. `library.db` is irreplaceable data — back up your `/database` volume before making any manual changes, and avoid editing the database while a Sync/Download/Verify job is running.
