...
> **Important:** Source is baked into the image at build time, so there's no live-reload or volume-mounted source. **Every code change requires a rebuild** (`up -d --build`) to be visible in the running container.
...
`pyproject.toml` sets `pythonpath = ["src"]` so the app package resolves without installing it, and `tests/conftest.py` redirects the config and database directories to temporary folders, so the suite never touches a real container or a real Audible account.
